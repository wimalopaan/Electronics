...
=== Verkabelung an das WeAct STM32 G031 Modul
...
[WARNING]
--
Auf dem `WeAct STM32 G031` sind zwei Lötbrücken: `SB1` und `SB2`. Diese beiden Brücken _müssen_ geschlossen werden, damit
die Pins `PC14` und `PC15` des µC mit den Anschlusspins des Boards verbunden werden.

*Achtung*: in den Bildern dieser Anleitung sind diese Brücken _noch_ nicht geschlossen worden.
--

.WeAct STM32G0B1 (Achtung: es fehlen auf diesem Bild noch die Lötbrücken, s.o.)
image::images/weact.jpg[]

.WeAct STM32G0B1 mit 3-poligem Versorgungsstecker (2-Pole belegt: Gnd, 5V) und 4-poligem SWD-Prorammierstecker (Achtung: es fehlen auf diesem Bild noch die Lötbrücken, s.o.)
...
| Stick 1 / ADC2 | gelb | A1
| Stick 1 / ADC3 | lila | A2
| Stick 1 / SW1 | weiß | C14
| Stick 1 / SW2 | blau | B7
| Stick 1 / Led | tbd (im Bild dieser Anleitung: hellblau) | A3
...
| Stick 2 / Led | tbd (im Bild dieser Anleitung: hellblau) | A8
| Stick 2 / Gnd | schwarz | Gnd
| Stick 2 / Vcc | rot | 3,3V

| SBus | tbd (im Bild dieser Anleitung: blau) | A11
|===
...
.Montage des µC-Moduls an der Rückseite des LCD-Displays (Achtung: es fehlen auf diesem Bild noch die Lötbrücken, s.o.)
image::images/weact_mount.jpg[]

.Gesamtantsicht von Innen (Achtung: es fehlen auf diesem Bild noch die Lötbrücken, s.o.)
...
Die digitalen Eingänge erscheinen als `TR9` bis `TR12`.

Die Trainer `TR7`, `TR8` und `TR13` bis `TR16` sind unbenutzt.
