...
[TIP]
--
All modifications can be selected via the `exresslrs-configurator`: simply select the pull-request `#3484` https://github.com/ExpressLRS/ExpressLRS/pull/3484[]
in expert-mode via the `git pull request`-tab.
...
One of the most outstanding feature of `ESCape32` is the possibility to use serial input (SBus(2), CRSF, SumDV3(HoTT), ...) and telemetry (CRSF, SBus2, S.Port, HoTT-Telemetry, ...). 
A markable feature ist the `Sbus2` protocoll, than combines control and telemetry data via one half-duplex line.
...
The project also defines special hardware targets (please contact if you have interest).
