...
[INFO]
--
This requires an external means to _activate_ the attached half-duplex devices (e.g. a button on the devices), because at most only _one_ device can be active on the bus (s.a. <<crsf-switch>>). 
--

[TIP]
--
If alle the devices obey a simple arbitration scheme, there could be more than _one active device_ on this half-dupley bus: 
