...
Although this appears to work but there a very serious problems!  

A little be of theory: the switching between these two signals can be seen as a convolution of each frequency with an according 
rectagular signal and then added together. This produces two main problems.

* The switching in the time-domain witch a rectangular signal or convolution in the frequency domain with a _si()_ function results in a very broad spectrum.
...
It turns out that this renders the rf part unusable, because several conventional receivers were not abel to decode the signal if the signal strength goes down. 
...
I looked around and I found some `27MHz` VXCO with an appropriate pulling range up to 100ppm. This looks quite reasonable: the µC could generate the cppm signal 
with some exponential roll-on / roll-off via its DAC. The VCXO clock signal is the used as the input for the SI5351. And the SI5351 simply generates the desired output
...
the smallest frequency sprectrum of the resulting rf signal.
...
image:stickng/stick1.png[width=240, link="stickng/stick1.png"]
