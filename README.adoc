...
[WARNING]
--
No AI generated code inside, all errors are human!
--
...
The basic processing of _source_ (like gimbals) via _inputs_, then _mixers_ and finally _outputs_ that was (is) present in `OpenTx` 
is still the same in `EdgeTx`. Therefore, the following images may help to understand that part of `EdgeTx`.

[NOTE]
--
The following images are of unknown copyright or licence. They are originally published here: https://opentx-doc.fr/wiki/diagrammes-de-fonctionnement[], 
the english version is of unknown origin (at least to me).
--

.Processing of inputs in EdgeTx
image::edgetx/images/inputs.png[width=240, link="edgetx/images/inputs.png"]

.Processing of mixers in EdgeTx
image::edgetx/images/mixes.png[width=240, link="edgetx/images/mixes.png"]

.Processing of outputs in EdgeTx
image::edgetx/images/outputs.png[width=240, link="edgetx/images/outputs.png"]
...
[[edgetx_32channels]]
=== 32-Channels extension for the CRSF-Protocol
...
A modified ExpressLRS can transport 32 channels: <<elrs-ch32>>
...
Precompiled EdgeTx (translation = `german`, includes also <<vcontrols>>) con be found here: https://github.com/wimalopaan/Electronics/tree/main/edgetx/firmware[]
...
[[crsf_batt_id]]
=== Enhanced Parser for instance-id augmented CRSF BATT-sensor frames

The CRSF protocol definition lacks a sensor type for sensing more than _one_ current measurement from a device like an ESC to
the handset. All crafts that have more than one motor / ESC would benefit if there would be a possiblility to 
send multiple current measurements from different devices (e.g. ESCs) to the handset.

Because there is no such sensor frame type in the CRSF protocol definition, it is kind of natural to extend that in a way it was done for 
e.g. RPM, temperature and voltages measurements.

A proposal to extend the CRSF protocol definition has been set: https://github.com/tbs-fpv/tbs-crsf-spec/issues/38[]

But there exists also another possiblilty: that `BATT` sensor can transport one current value together with a voltage measurement and 
a calculated consumed energie and percentage of accu capacity. Originally there is no possibility to send `BATT` sensor frames from different 
sources without confusion.

But the CRSF definition protocol itself allows to extend every frame definition by additional values / fields, if the overall validity of the frame 
is preserved. All conforming CRSF parsers (like in ExpressLRS to transport these frames) should process these frames flawlessly.

So, there is another proposal to the `BATT` sensor to contain an additional instance-id as the last `uint8_t`- field in this frame.

https://github.com/tbs-fpv/tbs-crsf-spec/issues/41[]

To parse these augmented `BATT` frames clearly _EdgeTx_ as the handset operating system has to be modified too:

https://github.com/EdgeTX/edgetx/pull/7571[]

This PR is already included in to the `wmextensions` branch of _EdgeTx_:

https://github.com/wimalopaan/edgetx/tree/wmextensions[]

If you want to test this, there is a PR to <<escape32>> to generate these augmented `BATT` frames: https://github.com/wimalopaan/ESCape32/tree/wm_crsf_batt_id[]
...
[TIP]
--
Precompiled EdgeTx (translation = `german`, includes also <<vcontrols>> and <<edgetx_32channels>>) con be found here: https://github.com/wimalopaan/Electronics/tree/main/edgetx/firmware[]
--
...
* controllability of all connected `CRSF` devices: the tx-module, the receiver, and _all_ connected devices (ESCs, MultiSwitch, Lighting-Module, ...) 
...
* dynamic power down to `10mW`
* very small and cheap receivers
...
multi-switch-modules (<<elrs_msw>>) or lighting-modules.
...
[TIP]
--
For all commands (command frame `0x32`) new _realms_  are defined:
...
===== Command `0x11` (intermodule pattern start)

This message type is intended to be send from one module to another, both are located _in_ the same model. 
So, it is required to have a CRSF-Router (or similar like ELRS-v4 receiver) these modules are connected to.

Typically, this is a _broadcast_ message (destination address: `0x00`), because the sending module does not
...
===== Command `0x12` (intermodule pattern stop all)
...
===== Command `0x13` (intermodule slave set)

This message type is intended to be send from one module to another, both are located _in_ the same model 
_or_ in two different module (like mother- and daughter boat or trauck and trailer).
So, it is required to have a CRSF-Router (or similar like ELRS-v4 receiver) these modules are connected to.

Typically, this is a _broadcast_ message (destination address: `0x00`), because the sending module does not
know the crsf-address of the destination module.

The purpose is to stet the outputs of the slave according to the master. The message should be send
from the master to the slave every time the state of the masters outputs change.
...
Send a CRSF frame over ESP-Now.

The _start-byte_ (`0xc8`), the fram-length field and CRC are omitted. They aren't needed.

To authorize the ESP-Now frames of a device, the `UID` bytes are prepended.

Overall `ESP-NOW` packet: `[uid5]` `[uid4]` `[uid3]` `[uid2]` `[uid1]` `[uid0]` `[type]` `[payload]`

`uid0` ... `uid5` are the bytes of _bind-phrase_ hash.

See also <<elrs-esp-now>>.
...
[TIP]
--
The modifications for ELRS V4 you'll find <<elrs-mods-v4>>.
--
...
* 32 channels extension <<elrs-ch32>>
* ability to flash / setup ESCape32 ESCs <<elrs-escape32>> (s.a. <<escape32>>)
* forward CRSF over ESP-NOW (s.a. <<elrs-esp-now>>)
...
The instructions to build ELRS V4 all hold as described in <<elrs_cs>> and <<elrs_compile>>.

[TIP]
--
The following modifications are unneccessary / useless for V4 of ExpressLRS because they are already included into V4:
...
[[elrs-ch32]]
==== 32-channels extension for ELRS V4

You can find the modification here:

https://github.com/wimalopaan/ExpressLRS/tree/wmextensions
...
https://github.com/wimalopaan/edgetx/tree/wmcrsfchanext
...
-DWMFILTER32
-DWMFILTER32W=500
# activate ESCape32 extension
# -DWMESCAPE32
# activate 3rd serial (Serial2) (only for ESP32)
# -DWMSERIAL2
----
...
[[elrs-escape32]]
==== Support for flashing / setup of `ESCape32` ESCs

`ESCape32` ESCs (s.a. <<escape32>>) are capable of reading input via CRSF and also produce telemetry via CRSF. They can be connected full-duplex to an
ExpressLRS receiver. Because they also deliver BEC to the receiver they are best connected to an ExpressLRS-PWM receiver (e.g. like Radiomaster ER6 or ER8). In this
case one has to configure Serial1 (the second serial interface in ExpressLRS) onto two of the PWM connectors, e.g. Output 8 for CRSF-TX and Output 7 for CRSF-RX. 
Then the Output 8 of the receiver must be connected to the signal input of the ESC and Output 7 of the reciver has to be connected to the telemetry pin of the ESC. 

But before this turns into live one has to configure the ESC to accept CRSF as input protocol and CRSF as telemetry protocol ((<<escape32>>). This normally happens
using the `ESCape32-WiFi-Link`. Clearly, you have to disconnect the ESC from the receiver to use the `WiFi-Link`.

You can avoid this manually changing cables (image a situation in a craft where you can hardly reach the ESC by hand) by using the ELRS
modifications: https://github.com/wimalopaan/ExpressLRS/tree/wmextensions

Be sure to include `-DWMEXTENSION` and `-DWMESCAPE32` 
...
in your `super_defines.txt`.

Switch your ELRS receiver to wifi-mode with the ESC connected (to Serial1) and you should notice a new menu-item `Escape32` in the menu bar:

.New menu-item for Escape32
image::elrs/escape32.png[width=240, link="elrs/escape32.png"]

You see the actual state of the ESC.

Select either a ESCape32 bootloader-image (be sure to pick the right one) or a firmware-image.
When you click `OK` in the file selection popup the flashing process starts. 

You should see a progress bar and a finish popup.

Afterward you can refresh the actual web-page and you should the new information (new state of the ESC).

Thats all.

[[elrs-3rd-serial]]
==== Third serial interface for ELRS V4

Sometimes you need even more serials! 

On ESP32 receivers ELRS can setup up to two serial connections. On PWM receivers there is one serial (the first one: `Serial`) connected
almost (at least: if unmodified hardware definition) to the JST-GH connector and the second serial (`Serial1`) may be mapped onto
two of the PWM connectors. 

This being said you can connect a FC or e.g. a _MultiSwitch_ via the JST-GH connector (this is absolutely fine because there is no 
massive current flowing hrough these wires for this purpose) _and_ at the same time connect e.g. an ESC (like << escape32>>) via 
`CRSF` in full-duplex. 

But what to do if you have more than one ESC? 

This situation is not uncommon in boats/ships and the like.

Well, you van use the _CruiseController_ (see below). But you also can make use of the third serial each `ESP32` contains.

Please find the source code here: https://github.com/wimalopaan/ExpressLRS/tree/wmextensions[]

Be sure to include `-DWMEXTENSION` and `-DWMSERIAL2` 
...
[[elrs-esp-now]]
==== CRSF-over-ESP-NOW for ELRS V4

This extension adds the _CRSF-over-ESP-NOW_ to ExpressLRS (s.a. <<crsf-esp-now>>).

With this extension one can use a dedicated ESP32-Module like the ESP32-C3-mini as a
_slave_ devices. The _master_ receiver forwards all incoming RF-packets via CRSF-over-ESP-NOW
to the _slave_ device. The slave device may then act as e.g. a MutliSwitch/E (see <<elrs_msw>>).

The ESP-NOW-Master is activated via the compilation options `-DWMEXTENSION` and `-DWMESPNOW`.

Aside from _slave_ devices like the ESP32-C3-mini one can also use a usual ELRS-receiver as slave
device. This is activatd via the compilation option `-DWMESPNOW_RECV`. If this is activated, the receiver
stops receiving rc-packets via LoRa-RF. Instead it then use ESP-NOW as the main input. Both the master receiver 
and the slave receiver _must_ use the same bind-phrase in order to authenticate each other. 

Using an normal ELRS-receiver as an _slave_ device, on can control _multiple_ models or parts of the 
same model at the same time (see <<multi-part-model>>). 

In contrast to the normal LoRa-RF connection between TX-module and ELRS-receiver (master), where only _one_ receiver
(with telemetry) could be connected, the ESP-NOW connection between the _master_ receiver and the _slave_ receiver
could be a 1-to-many. Therefore multiple slave receivers could be bound to a master receiver.

Be sure to include `-DWMEXTENSION` and `-DWMESPNOW`. If you want to use an
ELRS-receiver as an ESP-NOW
...
# activate ESP-NOW forwarding
-DWMESPNOW

# activate ESP-NOW receiver (disables esp-now forwarding, disables rf-connection)
-DWMESPNOW_RECV 
-DWMESPNOW_RECV_ADDR=0xEB
----

[[elrs-rx-relay]]
==== ELRS-receiver as CRSF-Relay in ELRS V4

t.b.d.

[[elrs-io-expand]]
==== ELRS-receiver with I2C IO-expander for MultiSwitch in ELRS V4

t.b.d.

==== ELRS V3 Modifications
...
[[elrs_sbus_flip]]
===== ELRS, 32-channels and SBus-Output (with channel flipping)  (Version 3.x.x only)
...
[[elrs_sumdv3]]
==== ELRS, SumDV3 and MultiSwitch (V4 and V3)
...
[[escape32]]
== ESCape32: firmware for a familiy of small/medium BLDC (and brushed) motor controller (brushless/brushed ESC)

`ESCape32` is a firmware for a family of brushless motor controller sharing a common hardware design (originated in the _BLHeli_-project). 
One of the most outstanding feature of `ESCape32` is the possibility to use a great variety of serial input (SBus(2), CRSF, SumDV3(HoTT), ...) 
and telemetry (CRSF, SBus2, S.Port, HoTT-Telemetry, ...). 
A markable feature is the `Sbus2` protocoll, that combines control and telemetry data via one half-duplex line.

As of version 17 the firmware supports full CRSF protocol. This means that you can configure all parameters of the 
firmware via the handset. Just fire up the ExpressLRS LUA (or TBS Agent), look for the ESC showing up under `Other devices` 
and go into that menu. You'll find all parameters in the menu.

[IMPORTANT]
--
To make use of the CRSF full-duplex be sure to configure the ESC to `input_mode == 5` (CRSF) and `telem_mode == 4` (CRSF)!
--

[WARNING]
--
Please be aware that you need an ESC that has the _input pin_ defined as `PA2` or `PA6` to get a _serial_ input via 
SBus/CRSF/SumDV3/IBUS. Targets with `PB4` (like the popular `SEQURE`) will not work. 

Please refer to https://github.com/neoxic/ESCape32/wiki/Targets[]
--

[WARNING]
--
Please also note that you need an exposed telemetry pin for _full-duplex_ CRSF to get full use of the CRSF-config-protocol.
--

This really simpifies configuration of your setup, especially when all components are built into the model. You 
don't have to pull-out a wire just to connect that to the WiFi-Link (or PC). Instead, all stays installed
and you can modify wihout event opening your craft.

The project: https://github.com/wimalopaan/ESCape32[ESCape32]

=== Sine startup

Link to link:bldc/escape32/esc.m[Octave code] for calculation.

=== Modifications

[[esc32_mod]]
==== Compile modified `ESCape32` in `github codespace`

I added an _Arch Linux_ `docker` configuration to my `ESCape32` 
repository: https://github.com/wimalopaan/ESCape32/tree/wm_codespace[]

To compile one of my branches of `ESCape32` like <<esc32_batt>> please refer to <<cs_wm>>.

[[esc32_batt]]
==== Augmented `BATT` telemetry frames (CRSF)

In this PR https://github.com/neoxic/ESCape32/pull/40[] you find a modification of `ESCape32` to produce
_augmented_ `BATT` frames. These frame now carry an additional `id` field. This is used to discriminate different 
`BATT` telemetry frames in the handset. 

If you already want to use this modification, you can use this branch https://github.com/wimalopaan/ESCape32/tree/wm_crsf_batt_id[]
and compile the firmware in a _codespace_: <<esc32_mod>>.

For `EdgeTx` you need this PR: <<crsf_batt_id>>.
...
The project also defines special hardware targets (please contact me if you have interest).

=== Info

I've a mini-series of videos in my YouTube-channel: please check them out!

[[inav]]
== INAV: Nagivation-enabled flight control software

I recently started to use also the `INAV` project. This journey begun realizing that
`INAV` also runs on the really beautiful `NexusX(R)` hardware from _Radiomaster_.

One of the first projects I did as _Mortimer_: <<mortimer>>

[[ieee]]
== IEEE: the RC software compound (stack)

Using `EdgeTx`, `ExpressLRS` and `ESCape32` is _the_ combination of RC related open-source software projects that I use 
now in all my crafts. 

[NOTE]
--
The three big `E`s: `EdgeTx`, `ExpressLRS` and `ESCape32` 
--

If I need something like _return-to-home_ I would choose clearly `INAV`. 
So, putting all together, you get `EEEI`.

[IMPORTANT]
--
If you push `EdgeTx`, `ExpressLRS`, `ESCape32` and `INAV` onto a stack, pop them off and take their first character, 
you'll get `IEEE`, the *RC software stack* (please do not confuse with https://www.ieee.org[], that's another crucial thing ;-) ).
--
...
[[oshwlab]]
=== Hardware designs available on OSHWLab

An increasing number of PCB designs are available on https://oshwlab.com/wilhelm.wm.meier/works[OSHWLab]. 
...
document: 
https://htmlpreview.github.io/?https://github.com/wimalopaan/Electronics/blob/main/rc/touch.html[X12S touch]
...
[[sfrog]]
=== SFrog 5D Sticks for the FrSky X12S

The following document decribes a modification for the X12S: 
https://htmlpreview.github.io/?https://github.com/wimalopaan/Electronics/blob/main/rc/sfrog.html[X12S SFrog 5D Stick modification] 
...
For the full story, please follow this 
https://htmlpreview.github.io/?https://github.com/wimalopaan/Electronics/blob/main/rc/jr40mhz.html[Howto (german)]
...
* on OpenI6X uninverted mode is compile-time option until version 2.1, afterwards is a runtime setting
...
===== Super simple MultiSwitch (ELRS/CRSF, FlySky/SBus, FlySky/IBus, FrSky/SBus, FrSky/S.Port, Graupner/SumDV3)
...
** select input (CRSF or SBUS or IBUS or S.Port or SumDV3)
...
communication is simply uni-directional.
...
If yout `SBUS` setup does not work, maybe because of the above mentioned obscure scalings, your next try may be S.Port. In the
...
There are also good news for the users of _Graupner_ (HoTT) stuff: the actor protocol `SumDV3` can transport up to 32 RC-channels 
and also 64 binary switches. Clearly you need a handset that is capable to define _digital switches_.
...
* Navigate to the directory: `boards/rcsupersimplemultiswitch`
* Edit the top lines in the file `settings.h`
** select input (CRSF or SBUS or IBUS or S.Port or SUMDV3)
** select if SBus is _normal_ or _inverted_
** select the rc-link (ELRS/AFHDS2A/ACCST for SBUS)
* use the command: `make -f Makefile all`
* download `ssmsw02.hex`
* connect the µC to the UPDI programmer (USBserial with a diode)
* flash, e.g.: `pymcuprog write --erase -d avr128da32 -t uart -u /dev/ttyUSB0 -f ssmsw02.hex`
...
** serial Servos (e.g. WaveShare ST3020) (see also: <<crsfserialservo>>)
...
The CRSF-switch acts in some respects like a LAN router/switch inkl. NAT: the attached devices need not to have different _crsf-addresses_, because the CRSF-switch
translates the addresses of the attached devices to a (predefined) set of addresses (used on the receiver side). So, the receiver _sees_ a point-2-point connection with a single device 
...
Broadcast messages or messages without address information like non-extended messages are forwarded to / from all devices. Be this means, RC-channel (or link-stats messages) data gets to all
connected devices as well as telemetry data (the forwarding can be configured via the CRSF-menu of the CRSF-router). 

The switch _learns_ the addresses of the attached devices by sending _CRSF-ping_ messages to all connected devices on startup. It decides if it is neccessary to translate the
address of an each attached device. 

The switch can act as a CRSF-relay as well (s.a. <<rcrelay32>>). Every connector can be used for a connecttion to an ELRS TX-module (half-duplex) or RX-as-TX (full-duplex).
...
Normally, one would use the RC-Desk at least with one serial connection to the handset. To to the
longer update intervall of the <<hwext>>, it might be useful to use a second (or the one and only) serial
connection that transports the proportional values via SBUS, and this can be used as SBUS-input / trainer-serial
in EdgeTx.
...
[[rcdesk32mini]]
=== The RC-Desk mini (for RadioMaster TX16S, TX15, FrSky X12S, X9E, ...)
...
`SBUS` channels 9 - 12. This requires to define _logical switches_ 
in EdgeTx to make use of the digital inputs to the `RC-Desk mini`.

Additionally to `SBUS` one can use `CRSF` protocoll or the <<hwext>> as a transport to the handset.
...
`CRSF` makes it also possible to transport the digital switch information via the <<crsf-sw>> protocol. So, 
an <<elrs_msw>> can be used as a multiswitch connected to the receiver or use an ELRS pwm receiver as a
multiswitch (<<elrs_pwm_msw>>).

The https://de.aliexpress.com/item/1005005748198584.html[WeAct STM32G031F8] is used as the hardware plattform.
...
| Analog Input 0 | PA0 | P2-7 | A0
| Analog Input 1 | PA1 | P2-8 | A1
| Analog Input 2 | PA2 | P2-9 | A2
| Analog Input 3 | PA3 | P2-10 | A3
| Analog Input 4 | PA5 | P3-7 | A5
| Analog Input 5 | PA6 | P3-6 | A6
| Analog Input 6 | PA7 | P3-5 | A7
| Analog Input 7 | PA8 | P3-4 | A8
| to handset aux tx | PA9 | P3-3  | A9 (A11)
| to handset aux rx | PA10 | P3-2  | A10 (A12)
| button on-board | PA14 |  | not exposed
| serial debug output | PA2 | P2-8 | A2 (collision with analog input 2) 
| LED (onboard) | PA4 | A4 | A4
| Switch input 0 | PB6 | P3-1 | B6
| Switch input 1 | PB7 | P2-1 | B7
| Switch input 2 | PC14 | P2-2 | C14
| Switch input 3 | PC15 | P2-3 | C15
|===

[[rcrelay32]]
=== RC-Relay32: a CRSF-Relay

(the following documentation is work-in-progess)

With the `CRSF-Relay` you can connect one CRSF-based RC-Link (like https://www.expresslrs.org[ExpressLRS] or TBS Crossfire or ...) comprising 
...
| unused | PA0 | P2-7 | A0
| unused | PA1 | P2-8 | A1
| to receiver tx | PA2 | P2-9 | A2
| to receiver rx | PA3 | P2-10 | A3
| unused | PA5 | P3-7 | A5
| unused | PA6 | P3-6 | A6
| unused | PA7 | P3-5 | A7
| unused | PA8 | P3-4 | A8
| to txmodule tx (half-duplex) | PA9 | P3-3  | A9 (A11)
| to txmodule rx | PA10 | P3-2  | A10 (A12)
| button on-board | PA14 |  | not exposed
| LED (onboard) | PA4 | A4 | A4
| unused | PB6 | P3-1 | B6
| unused | PB7 | P2-1 | B7
| unused | PC14 | P2-2 | C14
| unused | PC15 | P2-3 | C15
|===

[[relay_img01]]
.The Relay (click to enlarge)  
image::images/relay/relay01.jpg[width=240, link="images/relay/relay01.jpg"]

[[relay_img02]]
.The Relay (click to enlarge)  
image::images/relay/relay02.jpg[width=240, link="images/relay/relay02.jpg"]

[[relay_img03]]
.The Relay (click to enlarge)  
image::images/relay/relay03.jpg[width=240, link="images/relay/relay03.jpg"]

[[relay_menu04]]
.The Menu (click to enlarge)  
image::images/relay/menu04.jpg[width=240, link="images/relay/menu04.jpg"]

[[relay_menu01]]
.The Menu (click to enlarge)  
image::images/relay/menu01.jpg[width=240, link="images/relay/menu01.jpg"]

[[relay_menu02]]
.The Menu (click to enlarge)  
image::images/relay/menu02.jpg[width=240, link="images/relay/menu02.jpg"]

[[relay_menu03]]
.The Menu (click to enlarge)  
image::images/relay/menu03.jpg[width=240, link="images/relay/menu03.jpg"]

[[multi-part-model]]
==== The Multipart-Model problem

Actually it is kind of a challenge to control more than one RC-model at the same and using also telemetry. You may wonder who is doing such crazy stuff, but 
people want e.g. to control two ship-models (like the DGZRS-Cruisers and their daughter boats) or a RC-Crawler or RC-Truck together with its trailer. Often the RC-systems 
is kept running when the truck drives around with the trailer parked somehow.

So, it would be cool to have an RC-Link with a handset/transmitter-module to which more than one receiver using telemetry is connected to. But this is impossible, even with more than 
one transmitter-module in the handset, because there is no solution for multiple telemetry streams inside e.g. EdgeTx.

So, the CRSF-relay could be a solution: the handset/transmitter connects to the master-rc-model. Inside this you have a receiver as usual but also the CRSF-Relay connected to another
transmittre-module e.g. a very small and cheap ELRS rx-as-tx. This rx-as-tx connects to the slave-rc-model, where another receiver is placed in.

But the is also a much more _simpler_ solution: forward all packets that the receiver gets from the handset to a slave unit be some means that the receiver already has built-in. And for this puepose
_ESP-NOW_ comes into mind because every _ExpressLRS_ receiver is built upon an ESP32/ESP32C3/ESP8255! 

Therefore, if the distance between the main receiver and the slave unit is small like some centimeter between the RC-truck and the trailer or some <25m between the main ship and its daughter boat we can 
cut the chip count and use the ELRS-receiver also as an _ESP-NOW_ transceiver connected to a slave unit solely built on top of an e.g. ESP32C3mini dev-board.

The drawback of this solution is, that you need another modification of _ELRS_ that brings _ESP-NOW_ to the receiver <<elrs-esp-now>>.

[[devg0b1]]
=== WM STM32G0B1 development board

[[devg0b1img]]
.The WM STM32G0B1 (click to enlarge)  
image::images/wm/wmstm32g0b1.jpg[width=240, link="images/wm/wmstm32g0b1.jpg"]

The https://de.aliexpress.com/item/1005005748198584.html[WeAct STM32G031F8] is used for some of he here described projects. Although it is a 
nice dev-board for all kind of (small) projects, it has its limitations:
...
[[crsfserialservo]]
=== The CRSF to Serial-Servo Converter

This project is an adapter to use _serial servos_ together with an CRSF receiver.

_Serial servos_ aren't controlled via a PWM servo signal. Instead they use a serial data connection. Typically
each servo has a distinct address, so all servos can be connected to same same serial line. In this case
one has to use a 3-wire connector: Gnd, Vcc, data.

There are (have been) servos that use SBus as serial protocol. To configure the channel (address) they listen to
one has to use some configuration tool, the servo is connected too. This is very cumbersome.

In the robotics area other kind of servo is widely used as e.g. the WaveShare ST3020. The do have another form factor 
than the typical standard rc servos, but they can be equpped with a RC like servo arm. To mount them like a standard RC servo, 
one can use a 3D printed hat (see <<serialservo02>>).

This project impelements the WaveShare serial protocol, and gives the user a CRSF menu on the handset to configure
the servos. No disconnect to a configuration tool is required. The adjustments can be made even if the servo is 
already mounted inside the model.

The here used ST3020 servo is a 360° servo with feedback. 

With this servo the follwing features are possible:

* adjustable rotation angle between 1° and 12 * 360° (for full channel range)
* adjustable speed
* adjustable torque
* stall detection
* overload protection
...
(see also <<CC>> and <<rc720e>>)

Application of the serial-servo as sailboat winch: https://youtu.be/_jWBiEMZoZ8[Saudade].

[[serialservo01]]
.The WaveShare ST3020 serial servo (click to enlarge)  
image::images/waveshare/st3020.jpg[width=240, link="images/waveshare/st3020.jpg"]

[[serialservo02]]
.The WaveShare ST3020 serial servo connected together (click to enlarge)  
image::images/waveshare/st3020_4.jpg[width=240, link="images/waveshare/st3020_4.jpg"]

[[serialservo03]]
.The _CRSF to serial servo_ Adapter connected to 4 WaveShare ST3020 serial servos and an ELRS receiver (ER6) (click to enlarge)  
image::images/waveshare/st3020_adap.jpg[width=240, link="images/waveshare/st3020_adap.jpg"]

[[mortimer]]
=== Selft-balancing inverted pendulum robot with INav (Project: Mortimer)

Watching some videos about self-balancing robots I came up with the idea of building such a thing by myself 
but using my own way of getting the things done ;-)

Since I am using https://github.com/iNavFlight/inav[INav] on the Radiomaster https://radiomasterrc.com/products/nexus-xr-control-system[Nexus-XR]
in some other projects I found it natural to use it for this project too. INav has a bunch of free PID controllers built-in and the Nexus-XR contains
an ELRS-receiver and a IMU ready to use. So, it seems quite simple ...

Most inverted pendulum balancing-robots use some sort of brushed gear motors with encoders or stepper-motors I was first tempted to use
my own gimbal-motor driver. But since this was in an early state, this wasn't really a quick option.

I found these https://www.waveshare.com/wiki/DDSM210?srsltid=AfmBOoq7uD7-XkhQ9nspMRGSAIVBgr8Upqys0ygCCtGbGOxuoT5TQB3U[Waveshare DDSM210] motors. 
They seem ideally suited for the projectexcept for the fact, that they use an own serial protocol.

So I had to implement this in INav: https://github.com/wimalopaan/inav/tree/wm_ddsm210[INav with serial protocol for DDSM210]

After that it turns out that the control-loop in INav runs with 10Hz. This is far too slow.
I had to modify that too (a maximum of 4KHz). Setting the coefficients and additional parameters like filtering coefficients hard-coded
into the source is a bad idea. So, I decided to use some of the GVars in INav to set these parameters for the first two free PID controllers 
in INav.

* GVar[0]: gain 1/10 for PID controller 0
* GVar[1]: errorLPF cutoff 1/10 Hz for PID controller 0
* GVar[2]: dtermLPF cutoff 1/10 Hz for PID controller 0

* GVar[3]: gain 1/10 for PID controller 1
* GVar[4]: errorLPF cutoff 1/10 Hz for PID controller 1
* GVar[5]: dtermLPF cutoff 1/10 Hz for PID controller 1

All modifications are found in: https://github.com/wimalopaan/inav/tree/wm_pendulum[INav with modifications] for the pendulum-robot

https://www.youtube.com/watch?v=JkGJ32tAq5U[Explaining video] (german only)

https://youtu.be/_f_BZltnOyI[The robot in action]
...
[[detailed_toc]]
== Detailed table of contents (for reference)
