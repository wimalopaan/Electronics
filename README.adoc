...
Derzeit ist dafür das RC-CruiseControl Modul in Entwicklung. Dies hat folgende Möglichkeiten:
...
** Abfragen binärer Zustandsinformation wie bspw. Wassereinbruchsensoren
...
Das <<rccc>> Modul vereinigt sehr viele Funktionen in einem Board. Dies bringt den _Nachteil_ mit sich, dass 
bei größeren Modellen sehr viele Kabel zu diesem Modul geführt werden müssen. Im Sinne eines modularen Aufbaus 
kann es daher sinnvoller sein, die Gesamtfunktion in kleinere Module aufzuteilen, die dann im Modell nahe 
an den jeweiligen Aktoren / Sensoren verbaut werden. Diese kleineren Module werden dann durch ein Bus-System 
unterweinander und mit einem Master-Modul verbunden. Dies spart viel Verkabelungsaufwand.

AUs diesem Grund wird ebenfalls - unter Beibehaltung der in <<rccc>> genannten Funktionalität - eine Kombination 
aus einem _RC-Cruise-Master_ und dazugehörigen _Mezzanine-Modulen_ entwickelt, um damit Erfahrungen zu sammeln.

Das `RC-CruiseMaster`-Modul soll die folgenden Funktionen beinhalten:

* Schnittstelle zum Empfänger (CRSF, MAVLink)
** ggf. Montageplatz für den Empfänger
* CLI-Schnittstelle via BlueTooth, auch für _RoboRemo_-Protokoll basierte Steuerung
* S.Port-Schnittstelle zur Anbindung von S.Port-Sensoren/Aktoren
* SBus(2)/IBus-Schnittstelle zur Anbindung von Aktoren und Mezzanines (mehrere Steckplätze)
* Aux1/2-Schnittstelle: 
** GPS via NMEA
** high-speed Aktoren wie z.B. <<rchaptic>>
* Schnittstelle zu I2C-Mezzanine-Boards

Die folgenden Mezzanines werden entwickelt:

* `RC-SBus4-M`
** Umsetzung des SBus(2) in: 
*** Servo-PWM
*** Analog-Zeitmultiplex für alte, analoge Schaltmodule
*** Abfragen binärer Zustandsinformation wie bspw. Wassereinbruchsensoren
* `RC-Led16-M`
** Ansteuerung von 16 LEDs mit Konstantstromquellen
** Blink-Sequenzen
** Helligkeitsmodulation (Glühlampeneffekt)
* `RC-Switch8-M`
** Schaltausgänge bis 150mA
** einfache PWM-Modulation (Motorsteuerung)
